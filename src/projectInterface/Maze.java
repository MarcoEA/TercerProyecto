...
public class Maze extends JPanel implements MouseListener {
    ...
    //metodos para pausar o detener el hilo
    ...
    //metodos para eliminar los hilos
    ...
    public void deleteItemCoord(int coordY, int coordX) {
        for (int i = 0; i < items.size(); i++) {
            if (coordX == items.get(i).getCoordX() && coordY == items.get(i).getCoordY()) {
                items.get(i).brake();
                items.remove(i);
            }
        }
    ...
    public void animatedDeleteItemCoord(int coordY, int coordX) {
        for (int i = 0; i < items.size(); i++) {
            if (coordX == items.get(i).getCoordX() && coordY == items.get(i).getCoordY()) {
                try {
                    BufferedImage imageDestruction = ImageIO.read(new File("./ProjectImages/itemdestroyed.png"));
                    items.get(i).setImage(imageDestruction);
                    items.get(i).pause();
                } catch (IOException ex) {
    ...
    public void addBlock(int x, int y) throws IOException {
        ...
        repaint();
    }//verifica si hay un block o no y pinta
    ...
    //click izquierdo agrega o quita bloque, click derecho agrega item
    @Override
...
//Introduce los personajes (hilos) uno por uno evitando que pase uno encima del otro
